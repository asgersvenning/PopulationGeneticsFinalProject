...
precomputed <- if (precomputed != "force") file.exists(paste0("data/all_dmat", file_suffix, ".rds")) & precomputed else F
...
all_dmat_plt <- all_dmat %>%
  ...
  theme(
    ...
    legend.box.margin = margin(0,0,0,-4, "lines"),
    plot.margin = margin(1,2,0,0, "lines"),
  )

# all_dmat_plt
ggsave(paste0("all_dmat", file_suffix, ".pdf"), align_legend(all_dmat_plt),
       device = cairo_pdf, antialias = "subpixel",
       width = 7, height = 8, dpi = 300, scale = 1)
# all_dmat_plt
```
\begin{figure}
...
all_t_dmat <- all_dmat %>% 
  filter(!epas1 | (prs_1 %in% epas_set & prs_2 %in% epas_set)) %>% 
  arrange(region_1, region_2, country_1, country_2) %>% 
  mutate(
...
hier_cluster_dmat <- all_tree_tidy %>% 
  ...
  ggraph("dendrogram", height = if (!epas1 | TRUE) height else max(height) - height) +
  ...
  geom_node_tile(
    aes(fill = if (!epas1 | T) region else cl, 
        color = if (!epas1 | T) region else cl,
        width = leaf, 
        y = max_divergence),
    height = max_divergence / 17.5,
    linewidth = .1
  ...
  # Plot the name of the population above the squares
  geom_node_text(
    aes(color = if (!epas1 | T) region else cl, 
        label = if (!epas1 | T) region else cl,
        y = ifelse(leaf, max_divergence - max_divergence / 27.5, NA), 
        # Annoying text-rotation calculations
        angle = 360 - x/max(x) * 360 + 90 + 180 * ((x/max(x) * 360) > 180),
        # Text-justification has to be different on each side, since the text is flipped
        hjust = (x/max(x) * 360) <= 180),
  ...
  scale_fill_brewer(palette = if (!epas1) "Dark2" else "Set2") +
  scale_colour_brewer(palette = if (!epas1) "Dark2" else "Set2") +
  ...
  # Styling
  labs(
    title = if (figure_annotation == "") latex2exp::TeX("Neighbour-Joining Tree of the Whole-Genome Pairwise $d_{IoU}$ Matrix") else figure_annotation,
  ...
  theme(
    ...
    plot.title = if (figure_annotation == "") element_text(size = 32) else element_text(size = 48, hjust = 0.05, vjust = -10, margin = margin()),
    plot.margin = if (figure_annotation == "") margin(0,-.9,-1.75, -2, "cm") else margin(-2,-0.5,-1,-1.5, "cm")
  )

ggsave(paste0("hier_cluster_dmat", file_suffix, ".pdf"), hier_cluster_dmat,
       device = cairo_pdf, antialias = "default",
       width = 5.75, height = 6.25, scale = 2.5, dpi = 1000)
...
all_tree_tidy %>% 
  as_tibble() %>% 
  filter(name %in% names(arch_person_length)) %>% 
  mutate(
    region = if (!epas1) region else ifelse(country == "Tibet", "Tibet", region)
    ) %>%
  mutate(
    len = arch_person_length[name] * downscale
  ) %>% 
  ggplot(aes(len, height, color = if (!epas1) region else cl)) +
  geom_point() +
  geom_smooth(method = "lm") +
  scale_x_continuous(labels = prettify_scientific,
                n.breaks = 10)   +
  scale_color_brewer(palette = if (!epas1) "Dark2" else "Set3")

all_tree_tidy %>% 
  as_tibble() %>% 
  filter(name %in% names(arch_person_length)) %>% 
  mutate(region = if (!epas1) region else ifelse(country == "Tibet", "Tibet", region)) %>%
  mutate(
    len = arch_person_length[name] * downscale,
    region = factor(region)
  ) %>% 
  bam(height ~ len + len:region + s(region, bs = "re"), data = .,
      discrete = T) %>% 
  summary
```
