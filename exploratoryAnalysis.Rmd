...
arch_length_tot_plt <- arch %>% 
    ...
    ggplot(aes(tot_len, fill = region)) +
    geom_histogram(aes(color = after_scale(fill)),
                   bins = 100,
                   key_glyph = draw_key_point) +
    scale_fill_brewer(palette = "Dark2") +
    scale_x_continuous(labels = prettify_scientific,
                       # breaks = seq(0, 350, 50) * 10^6
                       ) +
    ...
    theme(
        ...
        legend.box.background = element_rect(colour = "black", linewidth = .5, linetype = "solid"),
        legend.box.margin = margin(1,1,1,1)
    ) +
    labs(x = "Total Archaic Segment Length", y = NULL, fill = NULL)
...
hier_cluster_dmat <- all_tree_tidy %>% 
  ...
  labs(
    ...
    # title = latex2exp::TeX("Neighbour-Joining Tree of pairwise $d_{IoU}$ matrix"),
...
ggsave(paste0("hier_cluster_dmat", file_suffix, ".pdf"), hier_cluster_dmat,
       device = cairo_pdf,
       width = 5.75, height = 5.5, scale = 2.5, dpi = 300)
